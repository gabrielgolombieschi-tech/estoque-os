...
export async function GET(req: NextRequest) {
  try {
    ...
    if (ueErr) return jerr(400, ueErr.message ?? "Erro ao carregar vendedores.");

    const candidates = (ueRows ?? [])
      .map((r) => r.usuario ?? null)
      .filter((u): u is UsuarioRow => Boolean(u?.id))
      ...
      .filter((u) => u.id && u.auth_user_id && (u.nome || u.email));

    const authUserIds = Array.from(new Set(candidates.map((u) => u.auth_user_id!).filter(Boolean)));
    if (authUserIds.length === 0) return NextResponse.json({ vendedores: [] });
    ...
    if (tmErr) return jerr(400, tmErr.message ?? "Erro ao validar vendedores no tenant.");
