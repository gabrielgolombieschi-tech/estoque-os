...
export default function NfeImportModal({
  ...
  const importXml = async () => {
    ...
    if (osSaldoExcedido) {
      return setError(
        `Valor da NF-e (R$ ${parsedTotal.toFixed(2)}) excede o saldo a faturar da OS (R$ ${(osSaldo?.saldo ?? 0).toFixed(2)}).`
      );
    }
          ...
          {file ? (
            <div className="space-y-2">
              <OsVinculoField
                ...
                disabled={busy || detectingCliente || checkingDuplicate || !ready}
                helperText="Obrigatorio. O valor desta NF-e sera abatido do saldo a faturar da OS vinculada."
              />
