...
import { CFOPS_ESTORNO, CFOPS_REMESSA_POR_FINALIDADE, FINALIDADES_REMESSA, textoOpcao } from "@/lib/fiscal/rotulos";
...
export default function OperacoesFiscaisClient() {
  ...
  return <main className="mx-auto max-w-[1500px] space-y-6 p-6 text-zinc-100">
    ...
    <section className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-5">
      ...
      {aba === "VENDA_ORDEM" && <div className="space-y-3"><h2 className="font-semibold">Venda à ordem — 6119 seguida de 6923</h2><p className="text-xs text-zinc-500">Saem duas notas com CFOP fixo: venda à ordem para quem compra <span className="font-mono">(6119)</span> e entrega por conta e ordem para quem recebe <span className="font-mono">(6923)</span>. Não há CFOP para escolher.</p><input className={field} value={ovId} onChange={e=>setOvId(e.target.value)} placeholder="ID da OV"/><div className="grid gap-2 md:grid-cols-3">{Object.entries(entrega).map(([key,value])=><input key={key} className={field} value={value} onChange={e=>setEntrega(s=>({...s,[key]:e.target.value}))} placeholder={`Entrega: ${key}`}/>)}</div><button disabled={busy||!ovId} className={button} onClick={criarVendaOrdem}>Criar as duas etapas</button></div>}
      ...
      {aba === "REMESSA_OUTRAS" && <div className="space-y-3"><h2 className="font-semibold">Remessa com controle de retorno (chave digitada)</h2><div className="flex flex-wrap gap-2"><select aria-label="Finalidade da remessa" className={field} value={finalidade} onChange={e=>{ const f=e.target.value; setFinalidade(f); setCfopRemessa(CFOPS_REMESSA_POR_FINALIDADE[f]?.[0]?.codigo ?? ""); }}>{FINALIDADES_REMESSA.map(o=><option key={o.codigo} value={o.codigo}>{o.rotulo}</option>)}</select><select aria-label="CFOP da remessa" className={field} value={cfopRemessa} onChange={e=>setCfopRemessa(e.target.value)}>{(CFOPS_REMESSA_POR_FINALIDADE[finalidade] ?? []).map(o=><option key={o.codigo} value={o.codigo}>{textoOpcao(o)}</option>)}</select><input className={field} value={destinatario.documento} onChange={e=>setDestinatario(s=>({...s,documento:e.target.value}))} placeholder="CPF/CNPJ destinatário"/><input className={field} value={destinatario.nome} onChange={e=>setDestinatario(s=>({...s,nome:e.target.value}))} placeholder="Nome destinatário"/></div><p className="text-xs text-zinc-500">{FINALIDADES_REMESSA.find(o=>o.codigo===finalidade)?.exemplo} {(CFOPS_REMESSA_POR_FINALIDADE[finalidade] ?? []).find(o=>o.codigo===cfopRemessa)?.exemplo} <span className="font-mono">CFOP {cfopRemessa || "?"}</span></p><textarea className={`${field} min-h-24 w-full font-mono`} value={itensRemessa} onChange={e=>setItensRemessa(e.target.value)}/><button disabled={busy} className={button} onClick={criarRemessa}>Criar remessa</button><div className="flex gap-2 border-t border-zinc-800 pt-3"><input className={field} type="number" min="1" value={prazo} onChange={e=>setPrazo(e.target.value)} placeholder="Prazo em dias (vazio = desligado; conserto: 180)"/><button className={button} onClick={salvarPrazo}>Salvar prazo da finalidade</button></div></div>}
      {aba === "ESTORNO" && <div className="space-y-3"><h2 className="font-semibold">Estorno espelhado da NF-e original</h2><input className={`${field} w-full`} value={documentoOriginal} onChange={e=>setDocumentoOriginal(e.target.value)} placeholder="UUID do documento fiscal original"/><select aria-label="Tipo do estorno" className={`${field} w-full`} value={cfopEstorno} onChange={e=>setCfopEstorno(e.target.value)}><option value="">O que está sendo estornado?</option>{CFOPS_ESTORNO.map(o=><option key={o.codigo} value={o.codigo}>{textoOpcao(o)}</option>)}</select><p className="text-xs text-zinc-500">{CFOPS_ESTORNO.find(o=>o.codigo===cfopEstorno)?.exemplo ?? "Escolha pelo tipo da nota original; o CFOP aparece entre parênteses."}</p><textarea className={`${field} min-h-20 w-full`} value={justificativa} onChange={e=>setJustificativa(e.target.value)} placeholder="Justificativa ao fisco"/><button disabled={busy} className={button} onClick={criarEstorno}>Criar estorno com finalidade 3</button></div>}
    </section>
