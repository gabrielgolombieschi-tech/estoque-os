...
export default function FluxoCaixaRealizadoClient() {
  ...
  const [rows, setRows] = useState<RealizadoRow[]>([]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const base = supabase
          .schema("f")
          .from("r_fluxo_caixa_realizado_diario")
          .select("data_ref,conta_bancaria_id,valor_realizado")
        ...
        if (qErr) throw qErr;
        if (!cancelled) setRows(((data ?? []) as unknown) as RealizadoRow[]);
      } catch (e: unknown) {
        if (!cancelled) {
          setRows([]);
          setError(e instanceof Error ? e.message : "Erro ao carregar fluxo realizado.");
        }
      } finally {
  ...
  const normalized = useMemo(() => {
    const items = rows.map((r) => {
      ...
      const value = n(r.valor_realizado);
  ...
  const exportCsv = () => {
    const header = ["data_ref", group === "dia" ? "" : "conta_bancaria", "valor_realizado"]
      .filter((h) => h.length > 0);
    ...
    downloadCsv(`fluxo_caixa_realizado_${range.start}_a_${range.end}.csv`, header, dataRows);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">Base caixa (pagamentos/recebimentos efetivos).</p>
        </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          <div>
            <label className="text-xs text-zinc-400" htmlFor="real-start">
              Início
            </label>
            <input
              id="real-start"
          ...
          <div>
            <label className="text-xs text-zinc-400" htmlFor="real-end">
              Fim
            </label>
            <input
              id="real-end"
          ...
          <div>
            <label className="text-xs text-zinc-400" htmlFor="real-conta">
              Conta (opcional)
            </label>
            <select
              id="real-conta"
          ...
          <div>
            <label className="text-xs text-zinc-400" htmlFor="real-group">
              Agrupar
            </label>
            <select
              id="real-group"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Stat title="Entradas (realizado)" value={formatMoney(normalized.entradas)} subtitle="Somatório de valores positivos" />
        <Stat title="Saídas (realizado)" value={formatMoney(normalized.saidas)} subtitle="Somatório de valores negativos" />
        <Stat title="Saldo (realizado)" value={formatMoney(normalized.saldo)} subtitle="Entradas + Saídas" />
      </div>
