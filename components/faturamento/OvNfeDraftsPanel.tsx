                    ...
                    <div className="space-y-5 p-5">
                      ...
                      <section className="rounded-xl border border-zinc-800 bg-zinc-900/20 p-5">
                        ...
                        <p className="mt-1 text-sm text-zinc-400">
                          ...
                          {" "}<Link href="/manuais/vender-peca-importada" target="_blank" className="text-sky-300 underline" data-testid="ajuda-vender-peca-importada">Manual: vender peça que nós importamos</Link>
                        </p>
                        ...
                        <fieldset disabled={autorizada || processando} className="space-y-5 border-0 p-0 disabled:opacity-70">
                          ...
                          {draft.itens.map((item) => {
                            ...
                            return <section key={item.id} className="space-y-4 rounded-lg border border-zinc-800 p-4">
                              ...
                              {resolvido?.status !== "RESOLVIDO" ? <div className="flex flex-wrap items-center justify-between gap-2 rounded border border-amber-800 bg-amber-950/20 p-3 text-sm text-amber-100"><span>{resolvido?.motivo || "Perfil fiscal não resolvido."}</span>{resolvido?.status === "PRODUTO_INCOMPLETO" && item.item_id ? (podeEditarFiscal ? (
                                /* De onde veio a peca decide a origem e o IPI: as duas portas, em linguagem simples. */
                                <span className="flex flex-wrap items-center gap-2">
                                  <Link href={`/itens?id=${item.item_id}&editar=1&aba=fiscal&retorno=/comercial/vendas/${ovId}`} className="rounded-md border border-amber-600 px-3 py-1.5 text-xs font-medium text-amber-100 hover:bg-amber-900/40">Compramos de distribuidor no Brasil</Link>
                                  <Link href={`/itens?id=${item.item_id}&editar=1&aba=fiscal&retorno=/comercial/vendas/${ovId}#importado-por-nos`} className="rounded-md border border-emerald-700 px-3 py-1.5 text-xs font-medium text-emerald-100 hover:bg-emerald-950/40">Importado por nós (direto ou conta e ordem)</Link>
                                </span>
                              ) : <span className="text-xs text-amber-200/80">Quem edita o fiscal dos itens resolve a origem no cadastro do item.</span>) : null}</div> : null}
