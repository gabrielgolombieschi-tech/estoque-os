...
export function validarCinquenta(m) {
  validarEscopo(m);
  assert.ok(["003", "004", "005"].includes(m.numero));
  assert.equal(m.itens.length, 50);
  assert.deepEqual(m.itens.map((i) => i.id).sort((a,b) => a-b), ({"003": ids003, "004": ids004, "005": ids005}[m.numero]).slice().sort((a,b) => a-b));
  for (const i of m.itens) {
    ...
    assert.equal(i.criterio, criterios[i.familia]);
...
export function exigirAutorizacao(m, aprovacao = null) {
  validarCinquenta(m);
  assert.notEqual(m.numero, "005", "Lote 005 aguarda aprovação humana; gravação bloqueada");
  if (m.numero === "003") {
  ...
  assert.equal(m.numero, "004");
  assert.equal(aprovacao.lote, m.lote);
  assert.equal(aprovacao.status, "aprovado");
  assert.equal(aprovacao.assinatura_lote, "8dfcc93fa2471aa24881f22ec41574c889f3b03bd5c7b6ab9996f3f3c7af0a26");
