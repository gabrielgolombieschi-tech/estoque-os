...
assert.equal(pendencias("CABO PARA SENSOR FOTOELÉTRICO 5M").length, 0, "Acessório não deve ser validado como sensor");
...
for (const item of manifesto.itens) {
  ...
  assert.equal(pendencias(item.nome, { grupoCodigo: item.grupo_codigo }).length, 0, item.codigo);
}
console.log("Qualidade de descrição: 20 verificações e 21 itens do manifesto aprovados.");
