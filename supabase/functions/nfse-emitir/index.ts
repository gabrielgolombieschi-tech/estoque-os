import { chamarFocus, focusConfigurado } from "../_shared/focus-nfe.ts";
import { adminClient, json, mensagemErro, responderOptions, userClient } from "../_shared/nfe-http.ts";
import { montarPayloadNfse, type ContextoNfse } from "../_shared/nfse-payload.ts";
import { aplicarRetornoNfse, normalizarFocusNfse } from "../_shared/nfse-retorno.ts";

/**
 * Emissao da NFS-e Padrao Nacional em HOMOLOGACAO (POST /v2/nfsen?ref=).
 * Mesma disciplina da NF-e: documento e emissao ja existem em RASCUNHO (com
 * a DPS numerada) antes de qualquer chamada externa; claim duravel; consulta
 * preventiva antes de qualquer novo POST na mesma referencia; retry depois de
 * rejeicao leva numero novo de DPS (o rejeitado fica queimado no log).
 *
 * A chamada a Focus fica atras de FOCUS_NFSE_NACIONAL_ENABLED=true, alem da
 * credencial de homologacao, porque a NFS-e Nacional precisa estar habilitada
 * no painel da Focus pelo responsavel.
 */

type Body = { solicitacao_id?: string };
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // 1. Documento RASCUNHO + emissao + DPS numerada (idempotente por referencia).
    ...
    if (!nfseNacionalHabilitada()) {
      return json({
        ...
        referencia: preparada.referencia_externa,
        status: "RASCUNHO",
        dps_serie: preparada.dps_serie,
        dps_numero: preparada.dps_numero,
        aguardando_habilitacao: true,
      }, 503);
    }
    ...
    if (emissao.ambiente !== "HOMOLOGACAO") throw new Error("Emissao bloqueada: o endpoint aceita somente HOMOLOGACAO.");
    ...
    if (houveTentativa) {
      const consulta = await chamarFocus(`/v2/nfsen/${encodeURIComponent(emissao.referencia_externa)}`);
      if (consulta.response.ok) {
        ...
        if (["AUTORIZADA", "PROCESSANDO"].includes(aplicado.retorno.status)) {
          return json({
            documento_fiscal_id: documentoId, referencia: emissao.referencia_externa, status: aplicado.retorno.status,
            idempotente: true, reconciliado_antes_do_retry: true,
          }, aplicado.retorno.status === "PROCESSANDO" ? 202 : 200);
        }
        // REJEITADA confirmada: segue para o retry com numero novo de DPS.
    ...
    // 4. Payload a partir do snapshot congelado (validacao local nomeando campo e cadastro).
    ...
    // 5. Claim duravel.
    const { data: claimData, error: claimError } = await admin.schema("f").rpc("fn_nfse_homologacao_claimar", {
      p_documento_fiscal_id: documentoId,
      p_payload: payload,
      p_reconciliacao_confirmada: reconciliacaoConfirmada,
    });
    if (claimError) {
      return json({ erro: claimError.message, documento_fiscal_id: documentoId }, claimError.code === "42501" ? 403 : claimError.code === "55000" ? 409 : 422);
    }
    ...
    // 6. POST na Focus.
    const chamada = await chamarFocus(`/v2/nfsen?ref=${encodeURIComponent(claim.referencia_externa)}`, { method: "POST", body: JSON.stringify(payload) });
    ...
    if (!chamada.response.ok) {
      ...
      const { error: registrarError } = await admin.schema("f").rpc("fn_nfe_registrar_envio", {
        ...
        p_status: rejeitada ? "REJEITADA" : "ERRO",
        p_codigo_status: chamada.response.status,
      ...
      // Recusa sincrona: a DPS reservada fica queimada no log (o retry renumera).
      await admin.schema("f").rpc("fn_nfse_dps_marcar", {
        p_documento_fiscal_id: documentoId,
        p_resultado: rejeitada ? "REJEITADO" : "ERRO",
        p_mensagem: normalizado.mensagem ?? `Focus respondeu HTTP ${chamada.response.status}.`,
      });
      return json({
        erro: normalizado.mensagem ?? "A Focus recusou a DPS.",
        codigo: chamada.response.status,
        documento_fiscal_id: documentoId,
        referencia: claim.referencia_externa,
        dps_serie: emissao.dps_serie,
        dps_numero: emissao.dps_numero,
    ...
    const { error: registrarError } = await admin.schema("f").rpc("fn_nfe_registrar_envio", {
      ...
      p_payload: payload,
      p_resposta: normalizado.bruto,
      p_status: normalizado.status === "CANCELADA" ? "ERRO" : normalizado.status,
    ...
    if (documentoId && claimVencedor && !respostaPersistida) {
      try {
        await admin.schema("f").rpc("fn_nfe_registrar_envio", {
          p_documento_fiscal_id: documentoId, p_payload: null, p_resposta: null, p_status: "ERRO", p_codigo_status: null, p_mensagem: mensagem,
        });
