...
begin
  if to_regclass('public.empresa_memberships') is not null then
    ...
    if to_regclass('public.tenants') is not null
      and not exists (select 1 from pg_constraint where conname = 'empresa_memberships_tenant_fkey') then
      alter table public.empresa_memberships
        add constraint empresa_memberships_tenant_fkey
        foreign key (tenant_id) references public.tenants(id) on delete cascade;
    end if;

    if to_regclass('public.empresas') is not null
      and not exists (select 1 from pg_constraint where conname = 'empresa_memberships_empresa_fkey') then
      alter table public.empresa_memberships
        add constraint empresa_memberships_empresa_fkey
        foreign key (empresa_id) references public.empresas(id) on delete cascade;
    end if;

    if to_regclass('auth.users') is not null
      and not exists (select 1 from pg_constraint where conname = 'empresa_memberships_user_fkey') then
      alter table public.empresa_memberships
        add constraint empresa_memberships_user_fkey
...
do $$
begin
  if to_regclass('public.user_empresa_context') is not null then
    if to_regclass('auth.users') is not null
      and not exists (select 1 from pg_constraint where conname = 'user_empresa_context_user_fkey') then
      alter table public.user_empresa_context
        add constraint user_empresa_context_user_fkey
        foreign key (user_id) references auth.users(id) on delete cascade;
    end if;

    if to_regclass('public.tenants') is not null
      and not exists (select 1 from pg_constraint where conname = 'user_empresa_context_tenant_fkey') then
      alter table public.user_empresa_context
        add constraint user_empresa_context_tenant_fkey
        foreign key (tenant_id) references public.tenants(id) on delete cascade;
    end if;

    if to_regclass('public.empresas') is not null
      and not exists (select 1 from pg_constraint where conname = 'user_empresa_context_empresa_fkey') then
      alter table public.user_empresa_context
        add constraint user_empresa_context_empresa_fkey
        foreign key (empresa_id) references public.empresas(id) on delete cascade;
    end if;
...
begin
  ...
  if has_criado_em then
    execute 'select id from public.tenants order by criado_em asc nulls last limit 1' into tenant_id;
  elsif has_created_at then
    execute 'select id from public.tenants order by created_at asc nulls last limit 1' into tenant_id;
  else
    execute 'select id from public.tenants limit 1' into tenant_id;
  end if;
...
begin
  if to_regclass('public.empresas') is not null
    ...
    and to_regclass('public.empresa_memberships') is not null then
...
begin
  if to_regclass('public.empresas') is not null
    and to_regclass('public.tenant_memberships') is not null
    and to_regclass('public.user_empresa_context') is not null then
    with default_empresas as (
      select distinct on (e.tenant_id) e.tenant_id, e.id as empresa_id
      from public.empresas e
      where e.ativo = true
