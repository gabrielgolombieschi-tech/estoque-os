-- Cancela pendencias de OS quando o item foi removido da OS
-- (nao existe mais os_itens ativo para a combinacao OS + item).
    ...
    with canceladas as (
      update m.compra_pendencia cp
         set status = 'CANCELADO',
             cancel_reason = 'Cancelado automaticamente: item baixado/removido da OS.',
       ...
       where cp.tenant_id = p_tenant_id
         ...
         and (
           ...
           or
           -- Item foi removido da OS ou zerado
           not exists (
             select 1
               from public.os_itens oi
              where oi.tenant_id = p_tenant_id
                ...
                and coalesce(oi.quantidade, 0) > 0
           )
         )
      returning cp.id
    )
