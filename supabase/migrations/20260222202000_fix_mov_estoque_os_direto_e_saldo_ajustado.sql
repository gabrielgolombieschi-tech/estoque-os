-- Fase 1: entrada com origem_os_id nao deve aumentar estoque
-- Fase 2: relatorio de entradas com saldo_ajustado para auditoria
...
begin
  ...
  v_delta :=
    case
      when new.tipo = 'entrada' and new.origem_os_id is not null then 0
...
$$;
-- Recalculo historico do estoque pela regra corrigida.
with saldo_calc as (
  select
    m.tenant_id,
    m.empresa_id,
    ...
    sum(
      case
        when m.tipo = 'entrada' and m.origem_os_id is not null then 0
...
as $$
  ...
  select
    ...
    coalesce(a.destino_os, '-') as destino_os,
    a.saldo_atual,
    (a.saldo_atual - a.qtd_para_os) as saldo_ajustado,
    a.estoque_ideal,
