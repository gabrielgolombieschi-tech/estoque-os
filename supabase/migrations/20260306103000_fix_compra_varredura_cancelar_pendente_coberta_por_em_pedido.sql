...
begin
  ...
  if p_incluir_os then
    -- Cancela pendencias OS que nao existem mais na origem.
    with fonte_os as (
      select
        oi.os_id,
        oi.item_id,
    ...
    -- Root fix: se ja existe EM_PEDIDO para a mesma OS+item, uma PENDENTE
    -- restante eh duplicada e deve ser cancelada.
    canceladas_cobertas as (
      update m.compra_pendencia cp
         set status = 'CANCELADO',
             cancel_reason = 'Cancelado automaticamente: ja coberta por pedido em aberto (EM_PEDIDO) para a mesma OS/item.',
             updated_by = a.fn_current_usuario_id()
       where cp.tenant_id = p_tenant_id
         and cp.empresa_id = p_empresa_id
         and cp.deleted_at is null
         ...
         and exists (
           select 1
           from m.compra_pendencia cp2
           where cp2.tenant_id = cp.tenant_id
             and cp2.empresa_id = cp.empresa_id
             and cp2.deleted_at is null
             and cp2.origem_tipo = 'OS'
             and cp2.status = 'EM_PEDIDO'
             and cp2.origem_os_id = cp.origem_os_id
             and cp2.item_id = cp.item_id
         )
      returning cp.id
    )
    select
      (select count(*) from canceladas_origem) + (select count(*) from canceladas_cobertas)
      into v_os_canceladas;
