...
-- A lista de OS do app passa a mostrar o valor gasto (custo operacional).
--
-- A formula ja existia dentro de public.get_os_lista_custos_operacionais, que a
-- tela web usa como "consumido". Em vez de copia-la para as RPCs do app, ela sai
-- para um nucleo unscoped e a funcao antiga passa a delegar — mesmo arranjo de
-- public.fn_preco_venda_item_unscoped (20260829110000_preco_venda_canonico.sql).
-- Assim existe uma unica copia da regra e web e app nunca divergem.
--
-- A formula, preservada exatamente como estava:
--   OS com relatorio HH : pedido_hh + materiais + despesas + mao_obra
--   demais OS           : materiais + despesas + mao_obra
--                         + orcado * (tipo_pedido = 'material' ? 0.27 : 0.15)
-- Ou seja, o componente de imposto so existe fora do fluxo HH; em OS HH o lugar
-- dele e ocupado pelo pedido de HH.

create or replace function public.fn_os_custo_operacional_unscoped(
  p_tenant_id uuid,
  p_empresa_id uuid,
  p_os_ids integer[]
)
...
begin
  if p_tenant_id is null
     ...
     or coalesce(array_length(p_os_ids, 1), 0) = 0 then
    return;
  end if;
...
revoke all on function public.fn_os_custo_operacional_unscoped(uuid, uuid, integer[])
  from public, anon, authenticated, service_role;

comment on function public.fn_os_custo_operacional_unscoped(uuid, uuid, integer[]) is
  'Nucleo do custo operacional da OS. Nao autoriza nada: os wrappers validam o acesso antes de chamar.';

-- Passa a delegar. O gate de papel continua identico ao que ja estava aqui.
...
begin
  ...
  from public.fn_os_custo_operacional_unscoped(p_tenant_id, p_empresa_id, p_os_ids) as custo;
end;
$$;
...
comment on function public.get_os_lista_custos_operacionais(uuid, uuid, integer[]) is
  'Retorna o custo operacional total de uma lista de OS usando a mesma formula da tela de detalhe.';
