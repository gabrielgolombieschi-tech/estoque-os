...
alter table public.itens
  add column if not exists unidade_compra character varying(10),
  add column if not exists fator_conversao_estoque numeric(18,6) not null default 1;

alter table public.itens
  drop constraint if exists itens_fator_conversao_estoque_check;

alter table public.itens
  add constraint itens_fator_conversao_estoque_check
  check (fator_conversao_estoque > 0);

comment on column public.itens.unidade_medida is
  'Unidade canonica do saldo de estoque. Para cabos sem terminacao, usar M.';

comment on column public.itens.unidade_compra is
  'Unidade comercial/origem usada pelo fornecedor ou XML, quando diferente da unidade canonica de estoque.';

comment on column public.itens.fator_conversao_estoque is
  'Quantidade na unidade de estoque correspondente a 1 unidade de compra. Ex.: 1 RL de 100 m = fator 100 com unidade_compra RL e unidade_medida M.';
...
returns table (
  ...
  unidade_medida text,
  unidade_compra text,
  fator_conversao_estoque numeric,
