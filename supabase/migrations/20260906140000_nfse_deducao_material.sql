-- Contador, 06/09/2026 (segunda rodada):
--  4. Abatimento de material na obra (07.02): LC 116/2003, art. 7, §2, I. O
--     material fornecido e incorporado a obra sai da base do ISS e do INSS. Na
--     DPS vai em "valor das deducoes" (Focus: valor_deducao_servico; motivo
--     Materiais). So em perfil com permite_deducao_material.
--  6. Frase do 14.01 com CRF, texto exato do contador.
--
-- Baselines: f.fn_nfse_discriminacao (20260906090000, 9 parametros),
-- f.fn_nfse_preparar_documento_solicitacao (20260905240000),
-- f.fn_os_nfse_conferir_homologacao (20260906120000).

alter table f.solicitacao_faturamento add column if not exists valor_deducao_material numeric(15,2);
alter table f.solicitacao_faturamento drop constraint if exists solicitacao_faturamento_deducao_material_ck;
alter table f.solicitacao_faturamento add constraint solicitacao_faturamento_deducao_material_ck check (valor_deducao_material is null or valor_deducao_material >= 0);
comment on column f.solicitacao_faturamento.valor_deducao_material is 'Material fornecido e incorporado a obra (07.02), deduzido da base do ISS e do INSS (LC 116/2003 art. 7 §2 I). Vai na DPS como valor das deducoes. Exige perfil com permite_deducao_material.';

-- ---------------------------------------------------------------------------
-- Discriminacao: token {MATERIAL}
-- ---------------------------------------------------------------------------
drop function if exists f.fn_nfse_discriminacao(jsonb, text, text, jsonb, date, boolean, text, text, text);
create function f.fn_nfse_discriminacao(
  ...
  p_material numeric default null  -- material deduzido da base (07.02)
) returns text
language plpgsql
immutable
...
declare
  v_template text := coalesce(nullif(btrim(coalesce(p_template, '')), ''),
    '{RESULTADO}|PEDIDO DE COMPRA: {PEDIDO}{ITEM}|VENCIMENTO: {VENCIMENTO} DDL|OS {OS}|MATERIAL APLICADO: {MATERIAL} (deduzido da base do ISS e do INSS, LC 116/2003, art. 7º, § 2º, I)|{FRASE_LEGAL}|{OBSERVACAO}');
  v_tokens jsonb;
  v_segmento text;
  v_texto text;
...
$$;
revoke all on function f.fn_nfse_discriminacao(jsonb, text, text, jsonb, date, boolean, text, text, text, numeric) from public;
grant execute on function f.fn_nfse_discriminacao(jsonb, text, text, jsonb, date, boolean, text, text, text, numeric) to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- Preparo: a deducao congelada na conferencia vai para a emissao
-- ---------------------------------------------------------------------------
create or replace function f.fn_nfse_preparar_documento_solicitacao(p_solicitacao_id uuid, p_ambiente text default 'HOMOLOGACAO')
...
begin
  ...
  values (v_documento_id, v_sf.id, v_sf.tenant_id, v_sf.empresa_id, v_referencia, p_ambiente, 'RASCUNHO', 'NFSE',
    v_dps.serie, v_dps.numero, v_serv->>'municipio_prestacao_ibge', (v_serv->>'iss_retido')::boolean, round((v_serv->>'valor_iss')::numeric, 2),
    coalesce(round((v_serv->>'valor_deducoes')::numeric, 2), 0),
...
-- ---------------------------------------------------------------------------
-- Conferencia: deducao de material (base do ISS e do INSS)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Frase do 14.01 com CRF: texto exato do contador (06/09/2026, segunda rodada)
-- ---------------------------------------------------------------------------
update f.tributacao_provisoria_nfse_homologacao set
  texto_com_retencao = 'SERVIÇO SUJEITO À RETENÇÃO DE CRF À ALÍQUOTA DE 4,65% (PIS 0,65%; COFINS 3,0%; CSLL 1,0%) CONFORME IN RFB N° 2.141/2023. TRIBUTOS INCIDENTES SOBRE O PREÇO CONFORME LEI 12.741/2012.'
