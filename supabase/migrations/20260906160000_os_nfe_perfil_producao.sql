-- NF-e de industrializacao (5101/6101) a partir da OS com perfil de producao.
-- Ate aqui a conferencia da OS so conhecia a fixture de homologacao (producao
-- fechada). Agora, quando existe perfil vigente e revisado para a combinacao
-- (natureza, ambito, UF, indIEDest, origem do item, destinacao, CRT), a linha
-- recebe os valores do perfil (tributacao_fonte = PERFIL, perfil_operacao_id
-- preenchido) e fn_nfe_producao_pronta passa a valer para a OS: homologar com o
-- perfil -> liberar (fn_perfil_operacao_nfe_liberar_producao) -> producao.
-- Sem perfil, a fixture continua valendo so para homologacao.
--
-- Perfil semeado (06/09/2026, contador: paineis 17% SC base cheia, IPI do NCM,
-- IPI na base do ICMS para consumidor final): SEG-IND-SC-5101-O0-CST00-17,
-- venda de producao propria dentro de SC para uso e consumo / ativo do
-- adquirente. Nasce em REVISAO, sem IBS/CBS revisado e sem producao.
--
-- Baseline: f.fn_os_nfe_conferir_homologacao (20260905170000).
...
select e.tenant_id, e.id, 'SEG-IND-SC-5101-O0-CST00-17',
  'SEG - venda de producao propria em SC - CFOP 5101 - origem 0 - CST 00 - 17% consumidor final',
  'NFE', 'VENDA_INDUSTRIALIZACAO_INTERNA', 'VENDA INDUSTRIALIZACAO DENTRO ESTADO', '3', '5101', null,
  '00', '01', '01', null, 0, 'SEM_BENEFICIO',
  'Contador 06/09/2026: paineis e sistemas fabricados (NCM 8537/8538/9032) saem a 17% dentro de SC com base cheia, sem reducao e sem cBenef; IPI e atributo do NCM no cadastro do item; para consumidor final o IPI integra a base do ICMS (NF-e 3766).',
  date '2026-09-06', 'REVISAO', 'Perfil de industrializacao propria; primeira nota assistida.', 0, 17.0000, 1.6500, 7.6000,
  false, 'INTERNA', array['SC'], '1', '3', 1, 1,
  array['USO_CONSUMO', 'ATIVO_IMOBILIZADO'], false, false
from c.empresa e
where e.codigo = 'SEG' and e.deleted_at is null
  and not exists (select 1 from f.perfil_operacao po where po.empresa_id = e.id and po.codigo = 'SEG-IND-SC-5101-O0-CST00-17');
