-- Perfil fiscal passa a poder valer so para certos NCMs.
--
-- Gabriel, 09/09/2026. Com o perfil de maquina industrial do Convenio 52/91 criado em
-- 20260909180000, passaram a existir DOIS perfis elegiveis para a mesma combinacao
-- (CFOP 5101, ativo imobilizado, origem 0, IE 1): o generico de CST 00 a 17% e o novo
-- de CST 20 com base reduzida. O desempate era "habilitado_producao desc", entao a
-- conferencia escolhia o generico — e o builder recusava a nota, porque para o NCM
-- 8479.81.90 ele exige CST 20 com cBenef e carga de 8,80%.
--
-- O que separa os dois nao e a operacao, e o PRODUTO: o beneficio do convenio vale
-- para maquinas e equipamentos industriais, listados por NCM. Entao o perfil ganha a
-- lista, e a escolha passa a considera-la:
--   * perfil com lista so e elegivel para os NCMs da lista;
--   * perfil sem lista continua sendo o generico da operacao;
--   * havendo os dois, o especifico por NCM ganha.

alter table f.perfil_operacao add column if not exists ncms text[];

comment on column f.perfil_operacao.ncms is
  'NCMs (so digitos) a que este perfil se aplica. Nulo ou vazio = perfil generico da operacao.';

update f.perfil_operacao
   set ncms = array['84609090', '84798190']
 where codigo = 'SEG-IND-SC-5101-O0-CST20-CONV5291'
   and ncms is distinct from array['84609090', '84798190'];
