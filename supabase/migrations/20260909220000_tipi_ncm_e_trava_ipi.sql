-- TIPI por NCM e trava de IPI na conferencia da NF-e.
--
-- Apontado pela contabilidade em 09/09/2026 sobre a NF-e 2/33 (homologacao, OS 288):
-- o item FAB-OS288-01 tem NCM tributado a 9,75% na TIPI e a nota saiu com CST 51 e
-- vIPI 0,00 — R$ 877,50 a menos. Nao foi erro de calculo: o motor ja destaca IPI (a
-- NF-e 2/7 da WEG saiu com vIPI 1.892,61); o cadastro do produto e que dizia CST 51
-- sem aliquota, e o motor obedeceu.
--
-- A tabela e a fonte da aliquota por NCM, para a conferencia parar de depender de
-- digitacao. So entram NCMs com origem declarada; NCM sem registro nao trava nada,
-- porque nao ha o que afirmar sobre ele.
--
-- A trava e na conferencia (e nao so no builder) porque e nela que os campos fiscais
-- do item sao resolvidos e onde o erro aparece na tela, antes de qualquer envio.

create table if not exists f.tipi_ncm (
  ...
  constraint tipi_ncm_ncm_ck check (ncm ~ '^[0-9]{8}$'),
  constraint tipi_ncm_aliquota_ck check (aliquota >= 0 and aliquota <= 100)
);

comment on table f.tipi_ncm is
  'Aliquota de IPI por NCM (TIPI). Usada pela conferencia da NF-e; NCM ausente nao gera trava.';

-- A conferencia grava a aliquota da TIPI no item para o builder poder repetir a trava
-- na hora de montar o payload, sem precisar consultar a tabela de dentro da edge.
alter table f.solicitacao_item add column if not exists ipi_tipi_aliquota numeric;
comment on column f.solicitacao_item.ipi_tipi_aliquota is
  'Aliquota de IPI da TIPI para o NCM do item, resolvida na conferencia. Nulo = NCM sem registro em f.tipi_ncm.';

insert into f.tipi_ncm (ncm, aliquota, descricao, fonte) values
  ('90328989', 9.7500, 'Instrumentos e aparelhos para regulacao ou controle automaticos - outros', 'Contabilidade da Segau, 09/09/2026'),
  ('90328990', 9.7500, 'Instrumentos e aparelhos para regulacao ou controle automaticos - residual', 'Contabilidade da Segau, 09/09/2026'),
  ('84798190', 0.0000, 'Maquinas e aparelhos para tratamento de metal por enrolamento de fios', 'Contabilidade da Segau, 09/09/2026'),
  ('85371019', 9.7500, 'Quadros, paineis e comandos para tensao nao superior a 1000 V', 'NF-e 3766 (agosto/2026) e fixture de homologacao')
