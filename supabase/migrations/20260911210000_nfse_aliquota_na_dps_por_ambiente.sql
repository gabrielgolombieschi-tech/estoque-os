-- A aliquota do ISS vai na DPS so onde o municipio de incidencia nao esta ativo no ambiente da emissao.
--
-- A NFS-e em homologacao da OS 139 (obra da WEG Tintas em Guaramirim) voltou com E0619 na DPS 2/22:
-- "E obrigatorio informar aliquota quando o prestador nao e optante do simples nacional, o municipio de
-- incidencia do ISSQN nao esta com situacao ATIVO no Sistema Nacional NFS-e e nao haja regime especial".
--
-- O montador nunca manda a aliquota, por causa do E0617 de 05/09/2026: onde o municipio esta ativo, o
-- ambiente nacional parametriza e recusa a aliquota informada por nao optante. E em PRODUCAO Guaramirim
-- esta ativo: as NFS-e 12 (03/08) e 47 (04/09/2026) foram autorizadas sem pAliq e o ambiente aplicou
-- 2,00 sozinho. O que muda e o ambiente: na homologacao (producao restrita) Guaramirim nao esta ativo.
--
-- A situacao e do municipio em cada ambiente, entao vira dado: f.nfse_aliquota_iss ganha
-- informar_na_dps_homologacao e informar_na_dps_producao (padrao false = municipio ativo, comportamento de
-- hoje). A conferencia grava as duas no snapshot do servico; o montador manda
-- percentual_aliquota_relativa_municipio so no ambiente marcado; e a comparacao producao x homologacao
-- aceita a diferenca nesse campo, e so nele.

alter table f.nfse_aliquota_iss
  add column if not exists informar_na_dps_homologacao boolean not null default false,
  add column if not exists informar_na_dps_producao boolean not null default false;

comment on column f.nfse_aliquota_iss.informar_na_dps_homologacao is
  'O municipio de incidencia nao esta ATIVO no ambiente nacional de homologacao: a DPS precisa trazer a aliquota (E0619). Padrao false: o ambiente parametriza e recusa a aliquota informada (E0617).';
comment on column f.nfse_aliquota_iss.informar_na_dps_producao is
  'O municipio de incidencia nao esta ATIVO no ambiente nacional de producao: a DPS precisa trazer a aliquota (E0619). Padrao false: o ambiente parametriza e recusa a aliquota informada (E0617).';

update f.nfse_aliquota_iss
   set informar_na_dps_homologacao = true,
       informar_na_dps_producao = false,
       fonte = fonte || ' Na DPS: homologacao informa a aliquota (Guaramirim inativo na producao restrita, E0619 na DPS 2/22 de 11/09/2026); producao nao informa (NFS-e 12 e 47 autorizadas sem pAliq).'
 where item_servico = '07.02' and municipio_ibge = '4206504'
   and not informar_na_dps_homologacao;
