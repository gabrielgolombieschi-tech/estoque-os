-- O material deduzido da NFS-e de obra passa a ser o material real da obra.
--
-- A primeira NFS-e de obra pelo ERP (OS 139, WEG Tintas, 11/09/2026) deduziu 50% de material, a
-- divisao da parcela anterior (NFS-e 1843). Somadas, as duas parcelas deduzem R$ 80.769,18 de material,
-- e a OS tem R$ 44.315,28 de produtos lancados. Decisao do Gabriel: "tem que pegar o real dos produtos
-- usados na obra; desta vamos seguir assim, mas ja ajusta para as proximas".
--
-- A regra, em f.fn_os_nfse_material_disponivel:
--   material real  = produtos lancados na OS (public.os_itens, itens do tipo produto), fora os de
--                    finalidade "venda" — esses saem por NF-e, e material vendido nao entra na deducao
--                    (contador, 06/09/2026);
--   ja deduzido    = NFS-e emitidas da OS (a deducao da emissao do ERP; nas importadas, material_valor ou
--                    a diferenca entre o servico e a base do ISS) + solicitacoes abertas que ainda
--                    reservam material (mesmo criterio de reserva do saldo da OS);
--   disponivel     = material real - ja deduzido, nunca negativo.
-- A conferencia bloqueia deducao acima do disponivel; a tela mostra a conta e sugere o disponivel.
--
-- A NFS-e 11 de homologacao da OS 139 fica como esta: a producao nao repete a conferencia.
--
-- Junto: a discriminacao imprime o material com o valor e o percentual do servico.

create or replace function f.fn_os_nfse_material_disponivel(
  p_tenant_id uuid,
  p_empresa_id uuid,
  p_os_ids integer[],
  p_excluir_solicitacao uuid default null
)
returns jsonb
language plpgsql
stable
...
begin
  ...
  select coalesce(round(sum(coalesce(
      nullif((select e.valor_deducoes from f.documento_fiscal_emissao e
              where e.documento_fiscal_id = d.id and e.status = 'AUTORIZADA'
              order by e.created_at desc limit 1), 0),
      nullif(d.material_valor, 0),
      greatest(d.valor_servicos - (select min(imp.base_calculo) from f.documento_fiscal_imposto imp
                                   where imp.documento_fiscal_id = d.id and imp.imposto = 'ISS'), 0),
      0)), 2), 0)
    into v_em_notas
  ...
  return jsonb_build_object(
    ...
    'material_disponivel', greatest(v_aplicado - v_em_notas - v_reservado, 0));
end;
$material$;

comment on function f.fn_os_nfse_material_disponivel(uuid, uuid, integer[], uuid) is
  'Material real da obra ainda nao deduzido em NFS-e: produtos lancados nas OS (fora os de venda) menos o ja deduzido em NFS-e emitidas e em solicitacoes abertas. Teto da deducao de material na conferencia da NFS-e.';

revoke all on function f.fn_os_nfse_material_disponivel(uuid, uuid, integer[], uuid) from public;
grant execute on function f.fn_os_nfse_material_disponivel(uuid, uuid, integer[], uuid) to authenticated, service_role;

-- Discriminacao: o material aplicado sai com o valor e o percentual do servico (pedido do Gabriel,
-- 11/09/2026: "era bom que viesse ao lado do valor o % aplicado de material"). O percentual precisa do valor
-- do servico, que a funcao nao recebia: novo parametro p_valor_servico. A assinatura antiga sai (so a
-- conferencia chamava), para a chamada com 10 argumentos nao ficar ambigua.
drop function if exists f.fn_nfse_discriminacao(jsonb, text, text, jsonb, date, boolean, text, text, text, numeric);
...
declare
  v_template text := coalesce(nullif(btrim(coalesce(p_template, '')), ''),
    '{RESULTADO}|PEDIDO DE COMPRA: {PEDIDO}{ITEM}|VENCIMENTO: {VENCIMENTO} DDL|OS {OS}|MATERIAL APLICADO: {MATERIAL} ({MATERIAL_PCT} do serviço), deduzido da base do ISS e do INSS (LC 116/2003, art. 7º, § 2º, I)|{FRASE_LEGAL}|{OBSERVACAO}');
  v_tokens jsonb;
  v_segmento text;
  v_texto text;
...
begin
  ...
  if v_deducao > 0 and v_permite_deducao and v_linhas > 0 then
    v_material := f.fn_os_nfse_material_disponivel(v_sf.tenant_id, v_sf.empresa_id,
      ...
      v_sf.id);
    if v_deducao > (v_material->>'material_disponivel')::numeric + 0.005 then
      v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','valor_deducao_material','mensagem',
        format('Material deduzido %s acima do material real da obra ainda nao deduzido (%s): produtos lancados na OS %s, fora os de venda, menos %s ja deduzidos em NFS-e desta OS. Lance na OS os produtos aplicados ou reduza a deducao.',
          f.fn_formatar_brl(v_deducao), f.fn_formatar_brl((v_material->>'material_disponivel')::numeric),
  ...
  if f_ctrib = any(array['070201','070202','070401','070501','070502','070601','070602','070701','070801','071701','071901','141403','141404']) then
    ...
    if v_obra_cno is not null then
      if char_length(v_obra_cno) > 30 then
        v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','obra','mensagem','CNO da obra acima de 30 caracteres.'));
      end if;
      v_obra := jsonb_build_object('codigo_obra', v_obra_cno);
