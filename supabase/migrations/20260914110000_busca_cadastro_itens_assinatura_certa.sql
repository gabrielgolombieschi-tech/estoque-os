-- Conserta o cadastro de itens, que a migration 20260914100000 quebrou.
--
-- O que aconteceu: peguei a definicao de search_cadastro_itens do baseline,
-- que tem 13 parametros, mas producao ja estava na versao de
-- 20260908131000_itens_fabricado_diferenciacao.sql, com 14 (p_fabricado) e tres
-- colunas a mais no retorno. Como create or replace so substitui quando a lista
-- de argumentos bate, o que eu fiz foi CRIAR UMA SOBRECARGA. A tela chama com
-- 13 argumentos nomeados, as duas candidatas serviam, e o Postgres recusou:
-- "Could not choose the best candidate function".
--
-- Aqui a sobrecarga de 13 vai embora e a versao de 14 — a de verdade — recebe a
-- busca sem acento, do mesmo jeito que as outras.
...
drop function if exists public.search_cadastro_itens(
  uuid, uuid, integer, text, text, text, text, text, boolean, integer, integer, text, text
);
...
declare
  ...
  v_produto_busca text := public.fn_texto_busca(v_produto);
    ...
    where i.tenant_id = p_tenant_id
      ...
      and (
        ...
        or (not v_codigo_exato and (
              public.fn_texto_busca(i.codigo_interno::text) like '%' || v_codigo_busca || '%'
              or public.fn_texto_busca(i.codigo_barras::text) like '%' || v_codigo_busca || '%'
            ))
      )
      and (v_produto is null or public.fn_texto_busca(i.nome::text) like '%' || v_produto_busca || '%')
