-- Importacao por remessa expressa: ajustes do Gabriel depois da primeira homologacao (NF-e 2/66, 17/09/2026).
--
-- 1. IBS/CBS na importacao: a base e a do II acrescida dos tributos do caput (LC 214/2025, art. 69,
--    caput e §§ 1º e 2º); o § 2º, II exclui o ICMS. Na remessa (RTS) fica valor aduaneiro + II, sem
--    ICMS e sem IPI. O banco passa a gravar `base_ibs_cbs` no snapshot (item e total) para o
--    montador conferir. Nesta nota: 700,75 (IBS 0,70, CBS 6,31).
-- 2. Trava de destino: CFOP 3101/3102 (industrializacao/revenda) e recusado quando a observacao ou
--    o motivo de compra indicam uso proprio (bancada, uso e consumo, ativo imobilizado, manutencao
--    interna...). Uso proprio e 3556 (consumo) ou 3551 (ativo).
-- 3. Entrada com origem 1 em 3101/3102 marca o item como importado pela Segau: fiscal_itens.origem 1,
--    origem_entrada 1 e equiparado_industrial (IPI destacado na saida futura). Em 3556/3551 nada.
--    O cancelamento da nota real desfaz a marca.
-- 4. Credito de ICMS da importacao nao e apropriado automaticamente: a GNRE esta em nome do courier
--    (UPS) e repassada na nota de debito. A movimentacao entra com credito_icms 0 e a importacao
--    guarda a pendencia (dados_json.icms_credito) para a contadora aprovar.

-- ---------------------------------------------------------------------------
-- 1 e 2. Criar a importacao: base IBS/CBS no snapshot e trava de uso proprio.
-- ---------------------------------------------------------------------------
create or replace function f.fn_importacao_remessa_uso_proprio(p_observacao text, p_motivo_codigo text, p_motivo_nome text)
returns text
language sql
immutable
set search_path to 'pg_catalog'
as $$
  -- Devolve o indicio de uso proprio (ou null). Observacao e motivo de compra, sem acentos e sem
  -- caixa: "bancada" (salvo do cliente), uso/consumo proprio ou interno, uso e consumo, ativo
  -- imobilizado, manutencao propria/interna/da fabrica, almoxarifado/ferramental proprio, sede,
  -- escritorio, laboratorio; motivos CONSUMO_*, MANUTENCAO_*, INVESTIMENTO, OPEX_*.
  select coalesce(
    case when coalesce(p_observacao, '') ~* '(uso|consumo)\s+(pr[oó]pri[oa]|intern[oa])|uso\s+e\s+consumo|ativo\s+imobilizado|imobilizado|bancada(?!\s+(do|da|de|para\s+[oa])\s+cliente)|manuten[cç][aã]o\s+(pr[oó]pria|interna|da\s+(f[aá]brica|empresa|sede))|almoxarifado\s+intern|ferramental\s+(pr[oó]pri|intern)|para\s+(a\s+)?(nossa|nosso|n[oó]s|a\s+segau)\b|escrit[oó]rio|laborat[oó]rio|\bsede\b'
      then 'observacao: "' || left(btrim(p_observacao), 80) || '"' end,
    case when coalesce(p_motivo_codigo, '') ~* '^(CONSUMO|MANUTENCAO|INVESTIMENTO|OPEX)' or coalesce(p_motivo_nome, '') ~* 'CONSUMO|USO\s+(E|OU)\s+CONSUMO|IMOBILIZADO|INVESTIMENTO|MANUTEN'
      then 'motivo de compra: ' || coalesce(p_motivo_nome, p_motivo_codigo) end
  );
$$;
...
revoke all on function f.fn_importacao_remessa_uso_proprio(text, text, text) from public, anon;
grant execute on function f.fn_importacao_remessa_uso_proprio(text, text, text) to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 3 e 4. Depois da nota real: credito de ICMS pendente da contadora; equiparacao a industrial
--        em 3101/3102 (desfeita no cancelamento).
-- ---------------------------------------------------------------------------
...
-- Reversao: reaplicar as definicoes de 20260918000000 (fn_importacao_remessa_criar e
--   fn_importacao_remessa_apos_emissao) e drop function f.fn_importacao_remessa_uso_proprio(text, text, text).
