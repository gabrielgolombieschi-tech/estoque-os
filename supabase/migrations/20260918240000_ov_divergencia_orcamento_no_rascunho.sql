-- OV cujas linhas nao somam o valor orcado: o rascunho de NF-e so nasce com o motivo.
--
-- Pedido do Gabriel em 18/09/2026, depois da OV-SEG-00004-026 (linha a 1.650,00 com orcado
-- de 4.563,40): a OV continua livre para editar, mas a criacao do rascunho de faturamento
-- para quando a soma das linhas (os_itens.valor_total) difere do orcado da OV, ate alguem
-- confirmar a diferenca com um motivo. O motivo, os dois valores, quem confirmou e quando
-- ficam na solicitacao (divergencia_orcamento). Sem diferenca, nada muda.
--
-- Assinatura da RPC ganha p_divergencia_motivo (default nulo). A antiga (6 parametros) e
-- removida, senao o PostgREST nao sabe qual escolher; a tela e o unico chamador.
-- Implementacao privada de 20260902122000, com o bloco novo antes do insert.
...
alter table f.solicitacao_faturamento
  add column if not exists divergencia_orcamento jsonb;

comment on column f.solicitacao_faturamento.divergencia_orcamento is
  'Quando a soma das linhas da OV difere do orcado na criacao do rascunho: soma_linhas, orcado, motivo, confirmado_por, confirmado_em (20260918240000).';

drop function if exists f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text);
drop function if exists f.fn_solicitacao_faturamento_criar_ov_impl(uuid, uuid, integer, jsonb, integer[], text);
...
create or replace function f.fn_solicitacao_faturamento_criar_ov_impl(
  ...
  p_divergencia_motivo text default null
)
returns uuid
language plpgsql
security definer
...
declare
  ...
  v_total_ids integer;
  v_soma_linhas numeric;
  v_orcado numeric;
  v_motivo text := nullif(btrim(coalesce(p_divergencia_motivo, '')), '');
  v_divergencia jsonb;
begin
  ...
  -- Linhas x orcado. A OV nao trava; o rascunho so nasce com o motivo da diferenca.
  -- OV sem orcado (sem orcamento fechado por tras) nao tem com o que comparar.
  select coalesce(sum(oi.valor_total), 0) into v_soma_linhas
  from public.os_itens oi
  where oi.tenant_id = p_tenant_id and oi.empresa_id = p_empresa_id and oi.os_id = p_os_id;
  v_orcado := v_origem.orcado;
  if v_orcado is not null and v_orcado > 0 and abs(v_soma_linhas - v_orcado) > 0.009 then
    if v_motivo is null or char_length(v_motivo) < 15 then
      raise exception using
        errcode = '22023',
        message = format(
          'As linhas da %s %s somam R$ %s e o orcamento fechado e R$ %s. Confira antes de faturar: informe o motivo da diferenca (15 caracteres ou mais) para criar o rascunho.',
          v_origem.tipo_documento, coalesce(v_origem.codigo, v_origem.numero_os),
          translate(to_char(v_soma_linhas, 'FM999,999,990.00'), ',.', '.,'),
          translate(to_char(v_orcado, 'FM999,999,990.00'), ',.', '.,')
        );
    end if;
    if char_length(v_motivo) > 500 then
      raise exception using errcode = '22023', message = 'O motivo da diferenca pode ter no maximo 500 caracteres.';
    end if;
    v_divergencia := jsonb_build_object(
      'soma_linhas', v_soma_linhas,
      'orcado', v_orcado,
      'motivo', v_motivo,
  ...
  insert into f.solicitacao_faturamento (
    ...
    observacao, natureza_operacao, divergencia_orcamento
  ) values (
    ...
    p_natureza_operacao, v_divergencia
  );
...
comment on function f.fn_solicitacao_faturamento_criar_ov_impl(uuid, uuid, integer, jsonb, integer[], text, text) is
  'Cria composicao em RASCUNHO e reserva saldo. Para OV, exige e grava o preco de venda explicitamente informado por linha; nunca usa o custo operacional. Linhas x orcado diferentes exigem p_divergencia_motivo.';

revoke all on function f.fn_solicitacao_faturamento_criar_ov_impl(uuid, uuid, integer, jsonb, integer[], text, text)
...
begin
  ...
  return f.fn_solicitacao_faturamento_criar_ov_impl(
    ...
    p_os_item_ids,
    p_natureza_operacao,
    p_divergencia_motivo
  );
end;
$function$;

comment on function f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text, text) is
  'Cria composicao parcial de OV por public.os_itens e reserva quantidade. OS deve usar linhas livres. Quando a soma das linhas difere do orcado, exige p_divergencia_motivo.';

revoke all on function f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text, text)
  from public, anon;
grant execute on function f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text, text)
  to authenticated, service_role;

do $assertions$
begin
  if to_regprocedure('f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text)') is not null then
    raise exception 'a assinatura antiga de fn_solicitacao_faturamento_criar_parcial continua existindo (PostgREST ficaria ambiguo)';
  end if;
  if to_regprocedure('f.fn_solicitacao_faturamento_criar_parcial(uuid, uuid, integer, jsonb, integer[], text, text)') is null then
    raise exception 'fn_solicitacao_faturamento_criar_parcial com p_divergencia_motivo nao existe';
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'f' and table_name = 'solicitacao_faturamento' and column_name = 'divergencia_orcamento'
  ) then
    raise exception 'coluna divergencia_orcamento nao criada';
  end if;
end;
$assertions$;
