-- Base do IBS/CBS da NFS-e: fora o ISS, o PIS e a COFINS proprios.
--
-- O ERP mostrava 55.100,00 na OS 298 (58.000 - ISS) porque o calculo foi calibrado nas NFS-e 32 e
-- 37 de agosto/2026, emitidas ANTES de a DPS levar PIS e COFINS proprios. Com os dois valores na
-- nota, o ambiente nacional devolveu no DANFSe da NFS-e de teste 21 (OS 298, 18/09/2026):
-- exclusoes 8.265,00 (ISS 2.900 + PIS 957 + COFINS 4.408), base 49.735,00, IBS 49,74, CBS 447,62.
-- E a regra da LC 214/2025, art. 12, § 2º.
--
-- Aqui a conferencia passa a calcular assim e a devolver tambem as exclusoes, para a tela mostrar
-- a conta. A definicao e a de 20260916120000 com essa unica mudanca (copiada do banco em
-- 18/09/2026). O mesmo ajuste foi feito no calculo em TypeScript (nfse-payload.ts), que serve a
-- previa e aos testes.
...
declare
  ...
  v_pis_proprio numeric(15,2) := 0; v_cofins_proprio numeric(15,2) := 0;
  ...
  v_obra jsonb; v_obra_cno text; v_obra_cep text;
  v_aliq_dps_hom boolean; v_aliq_dps_prod boolean;
  v_material jsonb;
begin
  ...
  begin
    v_competencia := coalesce(nullif(btrim(coalesce(p_operacao->>'data_competencia', '')), '')::date, f.fn_data_hoje_sao_paulo());
  ...
  elsif f.fn_nfse_competencia_pendencia(v_competencia, f.fn_data_hoje_sao_paulo()) is not null then v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','data_competencia','mensagem',f.fn_nfse_competencia_pendencia(v_competencia, f.fn_data_hoje_sao_paulo())));
  end if;

  if v_fonte is not null then
    select a.aliquota, a.informar_na_dps_homologacao, a.informar_na_dps_producao into f_aliq_iss, v_aliq_dps_hom, v_aliq_dps_prod from f.nfse_aliquota_iss a
  ...
  -- O material deduzido e o real da obra: os produtos lancados na OS (fora os de venda, que saem por
  -- NF-e e nao entram na deducao), menos o que ja foi deduzido em NFS-e desta OS. Decisao do Gabriel em
  -- 11/09/2026, depois que as duas parcelas do pedido 4518572701 somaram R$ 80.769,18 de material com
  -- R$ 44.315,28 lancados na OS 139.
  if v_deducao > 0 and v_permite_deducao and v_linhas > 0 then
    v_material := f.fn_os_nfse_material_disponivel(v_sf.tenant_id, v_sf.empresa_id,
      (select array_agg(distinct si.origem_id::integer) from f.solicitacao_item si where si.solicitacao_id = v_sf.id and si.origem_id ~ '^[0-9]+$'),
      v_sf.id);
    if v_deducao > coalesce((v_material->>'teto_deducao')::numeric, (v_material->>'material_disponivel')::numeric) + 0.005 then
      v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','valor_deducao_material','mensagem',
        format('Material deduzido %s acima do material real da obra ainda nao deduzido (%s): produtos lancados na OS %s, fora os de venda, menos %s ja deduzidos em NFS-e desta OS. Lance na OS os produtos aplicados ou reduza a deducao.',
          f.fn_formatar_brl(v_deducao), f.fn_formatar_brl(coalesce((v_material->>'teto_deducao')::numeric, (v_material->>'material_disponivel')::numeric)),
          f.fn_formatar_brl((v_material->>'material_aplicado')::numeric), f.fn_formatar_brl((v_material->>'material_deduzido')::numeric))));
    end if;
  end if;
  ...
  loop
    ...
    if v_sf.substitui_solicitacao_id is not null or v_sf.substitui_documento_fiscal_id is not null then
      ...
      select v_reserva_substituida + coalesce(sum(f.fn_documento_valor_faturado(d.modelo, d.valor_total, d.valor_servicos, d.valor_produtos)), 0) into v_reserva_substituida from f.documento_fiscal d
      where d.id = v_sf.substitui_documento_fiscal_id and upper(coalesce(d.nfse_status, '')) = 'EMITIDA' and d.os_id_import = v_os_total.os_id and d.deleted_at is null;
    end if;
  ...
  -- Local da obra (grupo obra da DPS). O ambiente nacional recusa sem ele os codigos de obra (E0370,
  -- DPS 2/20 da OS 139 em 11/09/2026). As NFS-e reais da WEG Tintas levam o endereco da fabrica em
  -- Guaramirim; a tela sugere o endereco do tomador e deixa trocar, ou informar o CNO, que tem prioridade.
  if f_ctrib = any(array['070201','070202','070401','070501','070502','070601','070602','070701','070801','071701','071901','141403','141404']) then
    v_obra_cno := nullif(btrim(coalesce(p_operacao->'obra'->>'codigo_obra', '')), '');
    v_obra_cep := regexp_replace(coalesce(p_operacao->'obra'->>'cep', ''), '[^0-9]', '', 'g');
    if v_obra_cno is not null then
      -- CNO tem 12 digitos (00.000.00000/00). O ambiente nacional aceitou "123" (NFS-e 13 de homologacao,
      -- 11/09/2026): quem erra a digitacao so descobriria na fiscalizacao.
      if regexp_replace(v_obra_cno, '[^0-9]', '', 'g') !~ '^[0-9]{12}$' then
        v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','obra','mensagem',
          format('CNO da obra "%s" invalido: o Cadastro Nacional de Obras tem 12 digitos (00.000.00000/00). Sem CNO, deixe o campo vazio e informe o endereco da obra.', v_obra_cno)));
      end if;
      v_obra := jsonb_build_object('codigo_obra', regexp_replace(v_obra_cno, '[^0-9]', '', 'g'));
    elsif v_obra_cep ~ '^[0-9]{8}$'
      and nullif(btrim(coalesce(p_operacao->'obra'->>'logradouro', '')), '') is not null
      and nullif(btrim(coalesce(p_operacao->'obra'->>'numero', '')), '') is not null
      and nullif(btrim(coalesce(p_operacao->'obra'->>'bairro', '')), '') is not null then
      v_obra := jsonb_build_object(
        'cep', v_obra_cep,
        'logradouro', left(btrim(p_operacao->'obra'->>'logradouro'), 255),
        'numero', left(btrim(p_operacao->'obra'->>'numero'), 60),
        'complemento', nullif(left(btrim(coalesce(p_operacao->'obra'->>'complemento', '')), 156), ''),
        'bairro', left(btrim(p_operacao->'obra'->>'bairro'), 60));
    else
      v_pend := v_pend || jsonb_build_array(jsonb_build_object('entidade','solicitacao','campo','obra','mensagem',
        format('Local da obra obrigatorio para o codigo de tributacao %s (o ambiente nacional recusa sem ele, E0370): informe o CNO ou o endereco da obra com CEP, logradouro, numero e bairro.', f_ctrib)));
    end if;
  end if;
  ...
  -- PIS e COFINS proprios (CST tributado): valor da nota, nao da retencao. Desde 18/09/2026 vao
  -- na DPS (valor_pis / valor_cofins) e por isso entram aqui.
  v_pis_proprio := case when coalesce(f_cst_pis, '') in ('01', '02') then round(v_bruto * coalesce(f_aliq_pis, 0) / 100, 2) else 0 end;
  v_cofins_proprio := case when coalesce(f_cst_cofins, '') in ('01', '02') then round(v_bruto * coalesce(f_aliq_cofins, 0) / 100, 2) else 0 end;
  -- Base do IBS/CBS (LC 214/2025, art. 12, § 2º): valor da operacao menos o ISS, o PIS e a COFINS.
  -- Desconto incondicional tambem sairia daqui; este fluxo nao tem esse campo hoje.
  -- Confere com o DANFSe devolvido pelo ambiente nacional na NFS-e de teste 21 da OS 298:
  -- 58.000,00 - 2.900,00 - 957,00 - 4.408,00 = 49.735,00, IBS 49,74 e CBS 447,62.
  v_ibs_base := v_bruto - v_valor_iss - v_pis_proprio - v_cofins_proprio;
  ...
  v_discriminacao := f.fn_nfse_discriminacao(v_linhas_desc, v_pedido, v_pedido_item, v_parcelas, f.fn_data_hoje_sao_paulo(), v_iss_retido, nullif(v_texto_retencao, ''),
    nullif(v_obs_composta, ''), v_cliente.nfse_discriminacao_template, v_deducao, v_bruto);
  ...
  set perfil_operacao_id = v_perfil.id, natureza_operacao = 'PRESTACAO_SERVICO', consumidor_final = v_consumidor_final,
      ...
      valor_deducao_material = nullif(v_deducao, 0), obra_dados = v_obra,
      ...
      operacao_snapshot = jsonb_build_object(
        ...
        'servico', jsonb_build_object(
          ...
          'ibs_cbs', jsonb_build_object('base', v_ibs_base, 'ibs_uf', v_ibs_uf, 'ibs_mun', v_ibs_mun, 'cbs', v_cbs, 'exclusoes', round(v_valor_iss + v_pis_proprio + v_cofins_proprio, 2), 'pis_proprio', v_pis_proprio, 'cofins_proprio', v_cofins_proprio, 'total', v_ibs_uf + v_ibs_mun + v_cbs, 'municipio_incidencia', v_municipio),
          ...
          || jsonb_build_object('obra', v_obra,
               'aliquota_iss_na_dps', jsonb_build_object('HOMOLOGACAO', coalesce(v_aliq_dps_hom, false), 'PRODUCAO', coalesce(v_aliq_dps_prod, false))),
  ...
  return jsonb_build_object(
    ...
    'previa', jsonb_build_object(
      ...
      'ibs_cbs', jsonb_build_object('base', v_ibs_base, 'ibs_uf', v_ibs_uf, 'ibs_mun', v_ibs_mun, 'cbs', v_cbs, 'exclusoes', round(v_valor_iss + v_pis_proprio + v_cofins_proprio, 2), 'pis_proprio', v_pis_proprio, 'cofins_proprio', v_cofins_proprio, 'total', v_ibs_uf + v_ibs_mun + v_cbs),
      ...
      'campos_conferir', v_perfil.campos_conferir, 'obra', v_obra));
end;
$function$;
...
begin
  if v_def not like '%v_ibs_base := v_bruto - v_valor_iss - v_pis_proprio - v_cofins_proprio;%' then
    raise exception 'base do IBS/CBS nao ficou sem PIS e COFINS';
  end if;
  if v_def not like '%''exclusoes'', round(v_valor_iss + v_pis_proprio + v_cofins_proprio, 2)%' then
    raise exception 'previa sem as exclusoes do IBS/CBS';
  end if;
end;
$assertions$;
