-- Fiscal e custo real - criacao de tabelas e colunas complementares
-- Idempotente para reexecucao segura
begin;

-- Funcao utilitaria para atualizar updated_at
...
begin
  if to_regclass('public.fiscal_itens') is not null
     and not exists (select 1 from pg_constraint where conname = 'fiscal_itens_item_id_key') then
...
begin
  if to_regclass('public.fiscal_itens') is not null
     and not exists (select 1 from pg_trigger where tgname = 'trg_fiscal_itens_updated_at') then
...
-- Constrangimento de nao nulo/unico para chave
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='nf_entrada' and column_name='chave'
  ) then
  ...
  if to_regclass('public.nf_entrada') is not null
     and not exists (select 1 from pg_constraint where conname = 'nf_entrada_chave_key') then
...
begin
  if to_regclass('public.nf_entrada') is not null
     and not exists (select 1 from pg_trigger where tgname = 'trg_nf_entrada_updated_at') then
...
begin
  if to_regclass('public.nf_entrada_itens') is not null
     and not exists (select 1 from pg_trigger where tgname = 'trg_nf_entrada_itens_updated_at') then
