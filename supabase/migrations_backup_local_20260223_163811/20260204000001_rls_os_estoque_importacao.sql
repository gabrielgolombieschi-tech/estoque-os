...
DROP POLICY IF EXISTS rls_ordens_servico_select ON public.ordens_servico;
DROP POLICY IF EXISTS rls_ordens_servico_insert ON public.ordens_servico;
DROP POLICY IF EXISTS rls_ordens_servico_update ON public.ordens_servico;
DROP POLICY IF EXISTS rls_ordens_servico_delete ON public.ordens_servico;

CREATE POLICY rls_ordens_servico_select ON public.ordens_servico
  FOR SELECT
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
    AND (
      public.has_permission('admin.all')
      OR public.has_permission('coord.all')
      ...
      OR public.has_permission('execucao.view')
    )
  );

CREATE POLICY rls_ordens_servico_insert ON public.ordens_servico
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_ordens_servico_update ON public.ordens_servico
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_ordens_servico_delete ON public.ordens_servico
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_os_gestao_itens_select ON public.os_gestao_itens;
DROP POLICY IF EXISTS rls_os_gestao_itens_insert ON public.os_gestao_itens;
DROP POLICY IF EXISTS rls_os_gestao_itens_update ON public.os_gestao_itens;
DROP POLICY IF EXISTS rls_os_gestao_itens_delete ON public.os_gestao_itens;

CREATE POLICY rls_os_gestao_itens_select ON public.os_gestao_itens
  FOR SELECT
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_gestao_itens_insert ON public.os_gestao_itens
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_gestao_itens_update ON public.os_gestao_itens
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_gestao_itens_delete ON public.os_gestao_itens
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_os_itens_select ON public.os_itens;
DROP POLICY IF EXISTS rls_os_itens_insert ON public.os_itens;
DROP POLICY IF EXISTS rls_os_itens_update ON public.os_itens;
DROP POLICY IF EXISTS rls_os_itens_delete ON public.os_itens;

CREATE POLICY rls_os_itens_select ON public.os_itens
  FOR SELECT
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_itens_insert ON public.os_itens
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_itens_update ON public.os_itens
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_os_itens_delete ON public.os_itens
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_itens_select ON public.itens;
DROP POLICY IF EXISTS rls_itens_insert ON public.itens;
DROP POLICY IF EXISTS rls_itens_update ON public.itens;
DROP POLICY IF EXISTS rls_itens_delete ON public.itens;

CREATE POLICY rls_itens_select ON public.itens
  FOR SELECT
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
    AND (
      public.has_permission('admin.all')
      OR public.has_permission('coord.all')
      OR public.has_permission('itens.view')
      OR public.has_permission('estoque.view')
    )
  );

CREATE POLICY rls_itens_insert ON public.itens
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
    AND (
      public.has_permission('admin.all')
      OR public.has_permission('coord.all')
      OR public.has_permission('itens.create')
    )
  );

CREATE POLICY rls_itens_update ON public.itens
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_itens_delete ON public.itens
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_fornecedores_insert ON public.fornecedores;
DROP POLICY IF EXISTS rls_fornecedores_update ON public.fornecedores;
DROP POLICY IF EXISTS rls_fornecedores_delete ON public.fornecedores;
...
CREATE POLICY rls_fornecedores_select ON public.fornecedores
  ...
  USING (
    ...
    AND (
      public.has_permission('admin.all')
      OR public.has_permission('coord.all')
      OR public.has_permission('fornecedores.create')
      OR public.has_permission('cadastros.fornecedores')
      OR public.has_permission('estoque.view')
    )
  );

CREATE POLICY rls_fornecedores_insert ON public.fornecedores
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_fornecedores_update ON public.fornecedores
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_fornecedores_delete ON public.fornecedores
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_estoque_select ON public.estoque;
DROP POLICY IF EXISTS rls_estoque_insert ON public.estoque;
DROP POLICY IF EXISTS rls_estoque_update ON public.estoque;
DROP POLICY IF EXISTS rls_estoque_delete ON public.estoque;

CREATE POLICY rls_estoque_select ON public.estoque
  FOR SELECT
  TO authenticated
  USING (
    ...
    AND (
      ...
      OR public.has_permission('coord.all')
      OR public.has_permission('estoque.view')
      OR public.has_permission('movimentacoes.view')
    )
  );

CREATE POLICY rls_estoque_insert ON public.estoque
  FOR INSERT
  TO authenticated
  WITH CHECK (
    empresa_id IS NOT NULL
    ...
    AND (
      public.has_permission('admin.all')
      OR public.has_permission('coord.all')
      OR public.has_permission('estoque.ajuste.create')
    )
  );

CREATE POLICY rls_estoque_update ON public.estoque
  FOR UPDATE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_estoque_delete ON public.estoque
  FOR DELETE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
DROP POLICY IF EXISTS rls_movimentacoes_select ON public.movimentacoes;
DROP POLICY IF EXISTS rls_movimentacoes_insert ON public.movimentacoes;
DROP POLICY IF EXISTS rls_movimentacoes_update ON public.movimentacoes;
DROP POLICY IF EXISTS rls_movimentacoes_delete ON public.movimentacoes;

CREATE POLICY rls_movimentacoes_select ON public.movimentacoes
  FOR SELECT
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_movimentacoes_insert ON public.movimentacoes
  FOR INSERT
  TO authenticated
  WITH CHECK (
    empresa_id IS NOT NULL
    ...
    AND (
      ...
      OR public.has_permission('coord.all')
      OR public.has_permission('estoque.ajuste.create')
      OR public.has_permission('nf_entrada.import')
    )
  );

CREATE POLICY rls_movimentacoes_update ON public.movimentacoes
  FOR UPDATE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_movimentacoes_delete ON public.movimentacoes
  FOR DELETE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
DROP POLICY IF EXISTS rls_nf_entrada_select ON public.nf_entrada;
DROP POLICY IF EXISTS rls_nf_entrada_insert ON public.nf_entrada;
DROP POLICY IF EXISTS rls_nf_entrada_update ON public.nf_entrada;
DROP POLICY IF EXISTS rls_nf_entrada_delete ON public.nf_entrada;

CREATE POLICY rls_nf_entrada_select ON public.nf_entrada
  FOR SELECT
  TO authenticated
  USING (
    empresa_id IS NOT NULL
    ...
    AND (
      ...
      OR public.has_permission('coord.all')
      OR public.has_permission('nf_entrada.import')
      OR public.has_permission('fiscal.edit')
    )
  );

CREATE POLICY rls_nf_entrada_insert ON public.nf_entrada
  FOR INSERT
  TO authenticated
  WITH CHECK (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_nf_entrada_update ON public.nf_entrada
  FOR UPDATE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_nf_entrada_delete ON public.nf_entrada
  FOR DELETE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
DROP POLICY IF EXISTS rls_nf_entrada_itens_select ON public.nf_entrada_itens;
DROP POLICY IF EXISTS rls_nf_entrada_itens_insert ON public.nf_entrada_itens;
DROP POLICY IF EXISTS rls_nf_entrada_itens_update ON public.nf_entrada_itens;
DROP POLICY IF EXISTS rls_nf_entrada_itens_delete ON public.nf_entrada_itens;

CREATE POLICY rls_nf_entrada_itens_select ON public.nf_entrada_itens
  FOR SELECT
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_nf_entrada_itens_insert ON public.nf_entrada_itens
  FOR INSERT
  TO authenticated
  WITH CHECK (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_nf_entrada_itens_update ON public.nf_entrada_itens
  FOR UPDATE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
CREATE POLICY rls_nf_entrada_itens_delete ON public.nf_entrada_itens
  FOR DELETE
  TO authenticated
  USING (
    tenant_id = public.current_tenant_id()
...
DROP POLICY IF EXISTS rls_fiscal_itens_select ON public.fiscal_itens;
DROP POLICY IF EXISTS rls_fiscal_itens_insert ON public.fiscal_itens;
DROP POLICY IF EXISTS rls_fiscal_itens_update ON public.fiscal_itens;
DROP POLICY IF EXISTS rls_fiscal_itens_delete ON public.fiscal_itens;

CREATE POLICY rls_fiscal_itens_select ON public.fiscal_itens
  FOR SELECT
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_fiscal_itens_insert ON public.fiscal_itens
  FOR INSERT
  TO authenticated
  WITH CHECK (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_fiscal_itens_update ON public.fiscal_itens
  FOR UPDATE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
CREATE POLICY rls_fiscal_itens_delete ON public.fiscal_itens
  FOR DELETE
  TO authenticated
  USING (
    empresa_id IS NOT NULL
...
-- RPCs / functions: add admin/coord bypass and permission checks
...
begin
  ...
  if not (
    ...
    raise exception 'Sem permissao para importar NF';
  end if;
  ...
  if coalesce(p_gerar_contas_pagar, false) then
    if not (
      public.has_permission('admin.all')
      or public.has_permission('coord.all')
      or public.has_permission('financeiro.gerenciar')
    ) then
      raise exception 'Sem permissao financeiro.gerenciar para gerar contas a pagar';
    end if;

    -- categoria "Compras (NF Entrada)" por tenant (cria se nao existir)
    ...
    -- Se nao veio parcelas_json, cria 1 parcela padrao (a vista)
  ...
  if v_has_os then
    ...
    loop
      ...
      -- baixa automatica: cria uma saida referenciando OS
      if p_baixar_os then
        insert into public.movimentacoes (
          item_id,
          tipo,
