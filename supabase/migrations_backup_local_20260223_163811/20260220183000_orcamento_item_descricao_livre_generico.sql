-- Permite descricao livre para item generico (codigo interno 9999) em orcamento_item.
-- Mantem comportamento atual para demais itens.
...
begin
  ...
  if v_tipo = 'produto' then
    ...
    raise exception 'Item do tipo "%" nao e permitido em Orcamento (somente produto/servico)', v_item.tipo;
  end if;

  new.item_nome := upper(trim(v_item.nome));
  new.unidade := upper(trim(coalesce(v_item.unidade_medida, 'UN')));

  -- Se for item generico (codigo 9999), usa a descricao livre por linha.
