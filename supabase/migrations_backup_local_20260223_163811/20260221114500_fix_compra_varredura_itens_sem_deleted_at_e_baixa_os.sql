-- Corrige varredura de compras para schemas sem itens.deleted_at
-- e cancela pendencias de OS quando o item ja foi baixado na OS.
...
begin
  ...
  if p_incluir_os then
    -- Remove pendencias OS que ja foram baixadas no fluxo de OS.
    with canceladas as (
      update m.compra_pendencia cp
         set status = 'CANCELADO',
             cancel_reason = 'Cancelado automaticamente: item ja baixado na OS.',
         ...
         and exists (
           select 1
            ...
            where oi.tenant_id = p_tenant_id
              ...
              and oi.os_id = cp.origem_os_id
              and oi.item_id = cp.item_id
              and coalesce(oi.baixa_estoque, false) = true
         )
      returning cp.id
    )
    ...
    with ins as (
      ...
      where oi.tenant_id = p_tenant_id
        ...
        and cp.id is null
        -- Nao abre compra quando estoque atual atende a quantidade da OS.
        and coalesce(e.quantidade_atual, 0) < oi.quantidade
      returning id
    )
